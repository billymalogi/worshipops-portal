...
import { 
  Calendar, Clock, Music, Mic2, Guitar, Loader2, CheckCircle, XCircle 
} from 'lucide-react';
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (positions && positions.length > 0) {
          ...
          // 4. Get Songs (And filter out empty ones!)
          if (nextService?.service?.id) {
            ...
            // CLEANUP: Filter out rows where title is empty/null
    ...
    <div className="max-w-2xl mx-auto space-y-8 font-sans">
            ...
            {nextUp.status === 'pending' && (
                <div className="grid grid-cols-2 gap-4">
                    <button 
                        ...
                        disabled={responding}
                        className="flex items-center justify-center gap-2 py-3 rounded-xl border border-gray-200 font-bold text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50"
                    >
                    ...
                    <button 
                        ...
                        disabled={responding}
                        className="flex items-center justify-center gap-2 py-3 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200 disabled:opacity-50"
                    >
      ...
      <div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          {setlist.length === 0 ? (
            <div className="p-8 text-center text-gray-400 italic">No songs published yet.</div>
          ) : (
